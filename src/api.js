...
export async function getCandidateByEmail(email) {
  ...
  if (!res.ok) {
    ...
    throw new Error(body?.message || `Error ${res.status} al buscar candidato`);
  }
...
export async function getJobsList() {
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => null);
    throw new Error(body?.message || `Error ${res.status} al obtener posiciones`);
  }
...
export async function applyToJob({ uuid, jobId, candidateId, applicationId, repoUrl }) {
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => null);
    throw new Error(body?.message || `Error ${res.status} al enviar postulación`);
  }
